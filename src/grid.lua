...
--- @param t any
--- @return Grid 
function Grid:fromTable(t) return Grid:fromLineIter(table.ipair_iter(t)) end
...
function Grid:fromLineIter(i)
    ...
    self.__index = self
    return
        setmetatable({data = data, width = line_width, height = height}, self)
...
function Grid:get_line(x, y, w)
    if x < 1 or x > self.width or y < 1 or y > self.height then return nil end
    if w < 1 or w + x - 1 > self.width then w = -x end
...
function Grid:rect_fill_iter(x, y, w, h, f)
    ...
    if x < 1 then
        ...
        return self:rect_fill_iter(1, y, w - to_fill, h, f):map(function(it)
            return f:rep(to_fill, "") .. it
        end)
    elseif y < 1 then
        local to_fill = 1 - y
        -- print("neg y", y,fh, to_fill )
        return Iter:rep(f:rep(w, ""), to_fill):append(
                   self:rect_fill_iter(x, 1, w, h - to_fill, f))
    elseif w < 1 then
        assert(false, "Negative width : " .. w)
    elseif h < 1 then
        assert(false, "Negative height: " .. h)
    elseif fw > self.width then
        local to_fill = fw - self.width
        -- print("wide", x,fw,to_fill)
        return self:rect_fill_iter(x, y, w - to_fill, h, f):map(function(it)
            return it .. f:rep(to_fill, "")
        end)
    elseif fh > self.height then
        local to_fill = fh - self.height
        -- print("tall", y,fh, to_fill )
        return self:rect_fill_iter(x, y, w, h - to_fill, f):append(Iter:rep(
                                                                       f:rep(w,
                                                                             ""),
                                                                       to_fill))
    end
...
function Grid:chars()
    return self:get_lines():product(
               function(line) return Iter:from_str(line) end)
end

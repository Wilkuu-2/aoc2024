...
local function init (args)
  ...
  local start = grid:coords():find(function(c) return grid:get(c.x,c.y) == "^" end)
...
function Point:from_table(t) 
  return setmetatable({x=t.x,y=t.y},  Point.mt)
end 
...
function Path:create(grid,start) 
  local p = {start=start, idir = 1, grid=grid, location=Point:new(start.x,start.y), steps={}} 
...
---@param p Point
---@return [boolean]|nil
function Path:get_step(p)
  return self.steps[p:to_string()]
end
...
function Path:step_add(p)
  ...
  local n = s ~= nil 
  if not n  then 
    self.steps[p:to_string()] = {[1] = false, [2] = false, [3] = false, [4] = false } 
    s = self:get_step(p)  
  end 
  s[self.idir] = true
  return n 
...
local function traverse(path, fn, log, add_obstacle) 
  ...
  while g ~= nil do 
    ...
    if node ~= nil and node[path.idir] then 
      if do_log then print("Found cycle at: (" .. next_loc:to_string() .. ")")  end 
      return nil 
    end 
...
local function part2(args) 
  ...
  local plen = traverse(path, function(npath) 
    ...
    if g ~= nil and dupe_set[il:to_string()] == nil and npath:get_step(il) == nil  then 

...
local function part1(args)
    return table.ipair_iter(args.state.lines):filter(function(l)
        return check_rule(args, l)
    end):fold(0, function(s, l) return s + l[math.ceil(#l / 2)] end)

end

local function part2(args)
    return table.ipair_iter(args.state.lines):filter(function(x)
        return (not check_rule(args, x))
    end):fold(0, function(s, x)
        return s + reorder_manuals(args, table.copy(x), 1)[math.ceil(#x / 2)]
    end)
end
